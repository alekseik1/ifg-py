...
from tests.conftest import set_up, temperatures_high, temperatures_st, volumes_st
...
class TestLowTemperaturesLimits:
    ...
    def test_C_T(self, temps, vols):
        with set_up(temps, vols) as (calc, (vv, tt)):
            expected = np.sqrt(
                10 / 9 * A * vv ** (-2 / 3) + beta / 9 * tt ** 2 * vv ** (2 / 3)
            )
    ...
    def test_C_S(self, temps, vols):
        with set_up(temps, vols) as (calc, (vv, tt)):
            expected = np.sqrt(
                10 / 9 * A * vv ** (-2 / 3) + 5 / 9 * beta * tt ** 2 * vv ** (2 / 3)
            )
...
class TestHighTemperaturesLimits:
    ...
    def test_F_potential(self, temps, vols):
        with set_up(temps, vols) as (calc, (vv, tt)):
            expected = (
                tt * np.log(1 / (g * vv) * (2 * np.pi / tt) ** (3 / 2))
                - tt
