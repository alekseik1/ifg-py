...
asymp1 = temps1 * (math.pow(np.pi / 3., 2. / 3.)) * math.pow(0.1, 2./3.)
asymp2 = temps1 * (math.pow(np.pi / 3., 2. / 3.)) * math.pow(1, 2./3.)
asymp3 = temps1 * (math.pow(np.pi / 3., 2. / 3.)) * math.pow(10, 2./3.)
vtmp = 0.1
asymp_cv_high = 1.5 - 3 * math.pow(np.pi, 1.5) / 8. / np.power(temps_cv_high, 1.5) / 2. / vtmp
asymp_cp_high = 1.5 - 3 * math.pow(np.pi, 1.5) / 8. / np.power(temps_cp_high, 1.5) / 2. / vtmp +\
                np.square(4. * np.power(temps_cp_high, 1.5) * 2 * vtmp - math.pow(np.pi, 1.5))\
                / 16. / np.power(temps_cp_high, 1.5) / 2. / vtmp\
                / (np.power(temps_cp_high, 1.5) * 2. * vtmp + np.power(np.pi, 1.5))
...
plt.plot(temps_cp_high, asymp_cp_high, '-',
         color='tab:orange', linewidth=1, label=r'$C_V$: high-$T$ asymp.')
plt.plot(temps_cv_high, asymp_cv_high, 'm-', linewidth=1, label=r'$C_P$: high-$T$ asymp.')
